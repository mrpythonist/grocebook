...
router.post('/add', async (req, res) => {
  try {
    ...
    if (brand.length > 13) {
      return res
        .status(400)
        .json({ error: 'Brand character length can not exceed 13.' });
    }
    ...
    await mailer.sendMail(email, 'merchant-application');
...
const createMerchantUser = async (email, name, merchant, host) => {
  ...
  if (existingUser) {
    ...
    await mailer.sendMail(email, 'merchant-welcome', null, name);
    ...
    await mailer.sendMail(email, 'merchant-signup', host, {
      resetToken,
      email
    });
